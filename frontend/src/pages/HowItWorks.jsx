              ...
              <Reveal key={s.n} delay={i * 0.08}>
                <div className="grid gap-6 lg:grid-cols-[auto_1fr] lg:items-start">
                  ...
                  <div className="rounded-3xl border border-line bg-white p-8">
                    <h3 className="font-serif text-3xl font-light text-ink">{s.title}</h3>
                    <p className="mt-3 max-w-xl text-base leading-relaxed text-[#666666]">{s.text}</p>
                  </div>
                </div>
              </Reveal>
            ))}
      ...
      <section className="container-x pb-24">
        <Reveal>
          <div className="noise relative overflow-hidden rounded-[2.5rem] bg-ink px-8 py-16 text-center text-white lg:px-16">
            ...
            <Link to="/register" className="relative z-10 mt-8 inline-flex items-center gap-2 rounded-full bg-gold px-8 py-4 text-sm font-medium text-white transition-all hover:bg-white hover:text-ink hover:-translate-y-1">Schedule now <ArrowRight className="h-4 w-4" strokeWidth={1.75} /></Link>
          </div>
        </Reveal>
      </section>
    </>
