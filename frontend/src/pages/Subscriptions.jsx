        ...
        <div className="container-x">
          ...
          <div className="mt-12 overflow-x-auto">
            <table className="w-full min-w-[720px] border-separate border-spacing-0" data-testid="plan-comparison-table">
              <thead>
                <tr>
                  <th className="p-4 text-left text-xs font-bold uppercase tracking-wider text-[#666666]">Feature</th>
                  {PLANS.map((p) => (
                    <th key={p.id} className={`p-4 text-center ${p.recommended ? "rounded-t-2xl bg-ink text-white" : "text-ink"}`}>
                      <p className="font-serif text-xl">{p.name}</p>
                      <p className={`text-xs ${p.recommended ? "text-gold" : "text-[#666666]"}`}>{ZAR(p.price)}/mo</p>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {ROWS.map((row, ri) => (
                  <tr key={row.label}>
                    <td className="border-t border-line p-4 text-sm font-medium text-ink">{row.label}</td>
                    {PLANS.map((p) => {
                      const val = row.get(p);
                      return (
                        <td key={p.id} className={`border-t border-line p-4 text-center text-sm ${p.recommended ? "bg-ink/[0.02]" : ""}`}>
                          {val === true ? <Check className="mx-auto h-4 w-4 text-gold" strokeWidth={2.5} /> : val === false ? <X className="mx-auto h-4 w-4 text-[#ccc]" strokeWidth={2} /> : <span className="text-ink">{val}</span>}
                        </td>
                      );
                    })}
                  </tr>
