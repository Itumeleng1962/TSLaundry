            ...
            <TableBody>
              {LAUNDRY_HISTORY.map((o) => (
                <TableRow key={o.id} className="border-line">
                  ...
                  <TableCell className="text-[#666666]">{o.date}</TableCell>
                  <TableCell className="text-[#666666]">{o.service}</TableCell>
                  <TableCell className="text-[#666666]">{o.kg}kg</TableCell>
            ...
            <TableBody>
              {INVOICES.map((i) => (
                <TableRow key={i.id} className="border-line">
                  <TableCell className="font-medium text-ink">{i.id}</TableCell>
                  <TableCell className="text-[#666666]">{i.date}</TableCell>
                  <TableCell className="font-medium text-ink">{ZAR(i.amount)}</TableCell>
            ...
            <TableBody>
              {PAYMENTS.map((p) => (
                <TableRow key={p.id} className="border-line">
                  ...
                  <TableCell className="text-[#666666]">{p.date}</TableCell>
                  <TableCell className="text-[#666666]">{p.method}</TableCell>
                  <TableCell className="font-medium text-ink">{ZAR(p.amount)}</TableCell>
            ...
            {NOTIFICATIONS.map((n) => {
              ...
              return (
                <div key={n.id} className={`flex items-center gap-4 rounded-xl border px-4 py-4 ${n.unread ? "border-gold/40 bg-gold/5" : "border-line"}`}>
                  ...
                  <div className="flex-1"><p className="text-sm font-medium text-ink">{n.title}</p><p className="text-xs text-[#666666]">{n.time}</p></div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Reveal>
          <div className="noise relative overflow-hidden rounded-2xl bg-ink p-8 text-white">
            ...
            <div className="relative z-10 mt-6 flex items-center gap-3 rounded-xl border border-white/15 bg-white/5 p-4">
        ...
        <Reveal delay={0.1}>
          <Panel title="Your rewards">
            <div className="space-y-4">
              <div className="flex items-end justify-between border-b border-line pb-4"><span className="text-sm text-[#666666]">Available credit</span><span className="font-serif text-4xl font-light text-gold">{ZAR(u.referralCredits)}</span></div>
              <div className="flex justify-between text-sm"><span className="text-[#666666]">Friends referred</span><span className="font-medium text-ink">4</span></div>
              <div className="flex justify-between text-sm"><span className="text-[#666666]">Pending invites</span><span className="font-medium text-ink">2</span></div>
              <div className="flex justify-between text-sm"><span className="text-[#666666]">Lifetime earned</span><span className="font-medium text-ink">{ZAR(480)}</span></div>
            </div>
          </Panel>
        </Reveal>
      </div>
...
export function Profile() {
  ...
  const cls = "w-full rounded-xl border border-line bg-white px-4 py-3 text-sm outline-none focus:border-gold";
  return (
    <>
        ...
        <Reveal>
          <Panel>
            <div className="flex flex-col items-center text-center">
              ...
              <p className="mt-4 font-serif text-2xl text-ink">{u.name}</p>
              <p className="text-sm text-[#666666]">{u.email}</p>
          ...
          <Panel title="Account details">
            <form onSubmit={handleSubmit(() => toast.success("Profile updated (demo)"))} className="space-y-4" data-testid="profile-form">
              <div className="grid gap-4 sm:grid-cols-2">
                <div><label className="mb-2 block text-xs font-bold uppercase tracking-wider text-[#666666]">Name</label><input {...register("name")} className={cls} /></div>
                <div><label className="mb-2 block text-xs font-bold uppercase tracking-wider text-[#666666]">Email</label><input {...register("email")} className={cls} /></div>
                <div><label className="mb-2 block text-xs font-bold uppercase tracking-wider text-[#666666]">Phone</label><input {...register("phone")} className={cls} /></div>
                <div><label className="mb-2 block text-xs font-bold uppercase tracking-wider text-[#666666]">Address</label><input {...register("address")} className={cls} /></div>
              </div>
