      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {ADMIN_STATS.map((s, i) => (
          <Reveal key={s.label} delay={i * 0.05}>
            <div className="rounded-2xl border border-line bg-white p-6" data-testid={`admin-stat-${i}`}>
              <p className="text-xs font-bold uppercase tracking-wider text-[#666666]">{s.label}</p>
              <p className="mt-2 font-serif text-3xl font-light text-ink">{s.value}</p>
          ...
          <Panel title="Revenue trend">
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={REVENUE_TREND} margin={{ left: 0, right: 8, top: 8 }}>
                ...
                <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid #ECECEC" }} formatter={(v) => ZAR(v)} />
          ...
          <Panel title="Plan distribution">
            ...
            <div className="mt-4 space-y-1.5">
              {PLAN_DISTRIBUTION.map((p, i) => (
                <div key={p.name} className="flex items-center justify-between text-xs"><span className="flex items-center gap-2 text-[#666666]"><span className="h-2.5 w-2.5 rounded-full" style={{ background: COLORS[i] }} />{p.name}</span><span className="font-medium text-ink">{p.value}</span></div>
              ))}
            </div>
          </Panel>
        </Reveal>
            ...
            <TableBody>
              {ADMIN_CUSTOMERS.map((c) => (
                <TableRow key={c.email} className="border-line">
                  ...
                  <TableCell className="text-[#666666]">{c.plan}</TableCell>
                  <TableCell className="font-medium text-ink">{ZAR(c.spend)}</TableCell>
            ...
            <TableBody>
              {ADMIN_ORDERS.map((o) => (
                <TableRow key={o.id} className="border-line">
                  ...
                  <TableCell className="text-[#666666]">{o.customer}</TableCell>
          ...
          <Panel title="FAQs" action={<button className="rounded-full bg-ink px-4 py-2 text-xs font-medium text-white">+ Add</button>}>
            <div className="space-y-2">
              {FAQS.slice(0, 5).map((f, i) => (
                <div key={i} className="flex items-center justify-between rounded-xl border border-line px-4 py-3"><span className="text-sm text-ink">{f.q}</span><span className="text-xs text-gold">Edit</span></div>
              ))}
            </div>
          </Panel>
        </Reveal>
            ...
            <div className="space-y-2">
              {TESTIMONIALS.map((t) => (
                <div key={t.name} className="flex items-center gap-3 rounded-xl border border-line px-4 py-3">
                  ...
                  <div className="flex-1"><p className="text-sm font-medium text-ink">{t.name}</p><p className="text-xs text-[#666666]">{t.role}</p></div>
